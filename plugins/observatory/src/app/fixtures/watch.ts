...
export function fixtureWatchSettings(): WatchSettings {
  return {
    mode: "observe",
    // Keys as `src/watch/settings.ts` names them, so the fixture page and the
    // live page render the same table.
    thresholds: {
      budget_perDayUsd: 500,
      budget_perTreeUsd: 50,
      watch_activeNoTurnMinutes: 10,
      watch_burnTokens: 150_000,
      watch_oscillationCycles: 2,
      watch_repeatCount: 3,
      watch_retryCount: 3,
      watch_silenceMinutes: 4,
    },
    source: {
      budget_perDayUsd: "setting",
      budget_perTreeUsd: "kv",
      watch_activeNoTurnMinutes: "setting",
      watch_burnTokens: "setting",
      watch_oscillationCycles: "setting",
      watch_repeatCount: "kv",
      watch_retryCount: "setting",
      watch_silenceMinutes: "setting",
    },
    note: null,
  };
}
